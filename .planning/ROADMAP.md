...
1. **MassTransit booking saga state machine** — implement the orchestration saga: `BookingInitiated` → `PriceReconfirmed` → `PNRCreated` → `PaymentAuthorized` → `TicketIssued` → `PaymentCaptured` → `BookingConfirmed`; persist saga state via `MassTransit.EntityFrameworkCore`; implement compensation chain for every step (PNR void on payment failure, Stripe refund on ticketing failure, payment release on GDS error); publish `BookingFailed` event with cause on any unrecoverable path
2. **Stripe Payment Service** — integrate Stripe Payment Intents with authorize-before-capture flow; process confirmation only via Stripe webhook (never trust client-side redirect); implement idempotency keys for all Stripe calls; store only `PaymentIntentId` and `CustomerId` — no raw card data; implement B2B credit wallet schema (`WalletTransactions` append-only log, `UPDLOCK`/`ROWLOCK` on balance reads); implement wallet top-up via Stripe for agency admins; isolate Payment Service so no other service accesses payment internals directly
3. **TTL monitor + compliance hardening** — implement background hosted service polling open PNRs every 5 minutes; extract ticketing deadline from fare rules (regex parser for IATA free-text formats); store TTL per PNR; fire saga hard-timeout at TTL minus 2 minutes; encrypt passport/document data at rest (AES-256); scrub PCI-sensitive fields from OpenTelemetry span attributes before export; store GDS credentials in environment secrets/vault only; enforce `[Authorize]` on all booking and account endpoints
4. **Notification Service + email templates** — consume `BookingConfirmed`, `BookingCancelled`, `TicketIssued` events from RabbitMQ; generate branded HTML email templates (flight confirmation with e-ticket attachment, hotel voucher, cancellation with refund details, B2B TTL deadline alerts, wallet low-balance alert); deliver via SendGrid/SMTP within 60-second SLA; use idempotency log to prevent duplicate sends; generate PDFs using QuestPDF
